...
function rewriteLegacyLinks(html: string) {
  ...
  // Serve modern WebP for every raster image under /uploads, /logo and /assets
  // (siblings generated by scripts/convert-to-webp.mjs). Covers src, href and the
  // hero <video poster>. The social card (og-image) and videos (.mp4) are left as-is.
  nextHtml = nextHtml.replace(
    /((?:src|href|poster)=")(\/(?:uploads|logo|assets)\/[^"]+?)\.(?:png|jpe?g)(")/gi,
    (match, pre: string, path: string, post: string) =>
      path.endsWith("/og-image") ? match : `${pre}${path}.webp${post}`,
  );
